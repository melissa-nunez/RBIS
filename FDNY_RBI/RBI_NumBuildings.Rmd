---
title: "Comparing Fire Inspections Before and After Risk Based Inspection Algorithm Normalized by Number of Buildings"
...
RBI.insp.ccd.bldg <- cd_sj %>% select(coun_dist, n, NumBldgs) %>% data.frame() %>% group_by(coun_dist, n) %>% summarize(NumBldgs=sum(NumBldgs)) %>% mutate(norm.count = n/NumBldgs)
...
fire.dispatch.1853.ccd.bldg <- dispatch.cd_sj %>% select(coun_dist, n, NumBldgs) %>% data.frame() %>% group_by(coun_dist, n) %>% summarize(NumBldgs=sum(NumBldgs)) %>% mutate(norm.count = n/NumBldgs)
...
fr.inspec.ccd.bldg <- fr.inspec.cd_sj  %>% select(coun_dist, n, NumBldgs) %>% data.frame() %>% group_by(coun_dist, n) %>% summarize(NumBldgs=sum(NumBldgs)) %>% mutate(norm.count = n/NumBldgs)
...
ggplot(lm_dat, aes(x=norm.count.fire, y=norm.count.insp)) +
  ...
  ggtitle("Plot of Fire Dispatches and RBI Inspections") + stat_cor(method = "pearson", label.x = 0.05, label.y = 1.10)
```
